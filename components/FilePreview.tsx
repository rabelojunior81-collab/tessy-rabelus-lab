...
const FilePreview: React.FC<FilePreviewProps> = ({ files, onRemove }) => {
  ...
  return (
    <div className="flex flex-wrap gap-3 animate-in slide-in-from-top-2 duration-300">
      ...
      {files.map((file) => (
        <div key={file.id} className="relative flex items-center gap-3 p-2 bg-white/80 dark:bg-slate-900/80 border-2 border-emerald-500/10 dark:border-emerald-500/20 shadow-[4px_4px_0_rgba(0,0,0,0.05)] dark:shadow-[4px_4px_0_rgba(0,0,0,0.3)] !rounded-none group hover:border-emerald-500/50 transition-all">
