          ...
          <div>
            ...
            <div className="space-y-4">
              {result.suggestions.map((s, idx) => (
                <div key={idx} className="p-4 bg-slate-800/40 border-l-4 border-amber-500 shadow-[4px_4px_0_rgba(0,0,0,0.2)]">
                  <span className="text-[9px] font-black px-2 py-0.5 bg-amber-500/20 text-amber-400 uppercase tracking-widest mb-2 inline-block">
                    {s.category}
                  </span>
                  <p className="text-sm text-white font-bold mb-1">{s.issue}</p>
                  <p className="text-xs text-slate-400 italic">Recomendação: {s.recommendation}</p>
                </div>
              ))}
            </div>
          </div>
