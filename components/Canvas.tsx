...
const Canvas: React.FC<CanvasProps> = ({ 
  ...
  return (
    <div className="h-full flex flex-col p-8 bg-transparent overflow-hidden relative">
        ...
        <div className="flex items-center gap-3">
          ...
          {(result || conversationHistory.length > 0) && !isLoading && (
            <>
              ...
              <button onClick={handleCopy} className={`brutalist-button text-[10px] px-3 py-2 font-black uppercase tracking-widest flex items-center gap-2 ${copied ? 'bg-emerald-500 text-white' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400'}`}>
      ...
      <div ref={scrollContainerRef} className="flex-1 w-full glass-panel !rounded-none p-8 overflow-y-auto custom-scrollbar flex flex-col space-y-8 bg-white/30 dark:bg-slate-900/30">
        {conversationHistory.length === 0 && !isLoading && !result && !pendingUserMessage && (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 italic text-center px-12">
        ...
        {conversationHistory.map((turn) => (
          <div key={turn.id} className="flex flex-col space-y-4">
            <div className="self-end max-w-[90%] bg-emerald-500/5 dark:bg-emerald-500/10 border-2 border-emerald-500/30 dark:border-emerald-500/40 p-4 rounded-none text-sm text-slate-800 dark:text-white shadow-[6px_6px_0_rgba(0,0,0,0.05)] dark:shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
              ...
              {turn.attachedFiles && turn.attachedFiles.length > 0 && (
                 <div className="mt-3 flex gap-2 flex-wrap">
                    {turn.attachedFiles.map(f => (
                       <div key={f.id} className="text-[9px] bg-emerald-500/20 dark:bg-emerald-500/30 px-2 py-1 border border-emerald-500/40 dark:border-emerald-500/50 font-black uppercase text-emerald-800 dark:text-emerald-200">📎 {f.name}</div>
                    ))}
                 </div>
              )}
            </div>
            <div className="self-start max-w-[90%] bg-white/70 dark:bg-slate-800/60 backdrop-blur-xl border-2 border-emerald-500/20 p-5 rounded-none text-sm text-slate-800 dark:text-emerald-50 leading-relaxed shadow-[6px_6px_0_rgba(0,0,0,0.05)] dark:shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
        ...
        {pendingUserMessage && (
          <div className="flex flex-col space-y-4 animate-pulse">
            ...
            <div className="self-start max-w-[90%] bg-white/40 dark:bg-slate-800/40 border-2 border-emerald-500/10 p-5 text-sm text-emerald-600 dark:text-emerald-400 uppercase font-black tracking-widest flex items-center gap-4">
        ...
        {result && !isLoading && (
           <div className="self-start max-w-[90%] bg-red-600/10 dark:bg-red-600/20 border-2 border-red-600/30 dark:border-red-600/40 p-5 text-sm text-red-700 dark:text-red-300 font-bold shadow-[6px_6px_0_rgba(0,0,0,0.05)] dark:shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
      ...
      <div className="mt-8 flex flex-col space-y-4 shrink-0 z-10">
        ...
        <div className="flex items-center gap-4 bg-white/60 dark:bg-slate-900/40 backdrop-blur-2xl p-4 border-2 border-emerald-500/20 shadow-[8px_8px_0_rgba(0,0,0,0.05)] dark:shadow-[8px_8px_0_rgba(0,0,0,0.5)] focus-within:border-emerald-500/50 transition-all">
          <button onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all border-2 border-transparent hover:border-emerald-500/30" title="Anexar">
          ...
          <textarea
            ...
            className="flex-1 bg-transparent border-none py-2 px-2 focus:outline-none text-base text-slate-900 dark:text-white placeholder-emerald-900/30 dark:placeholder-emerald-900/50 font-bold resize-none h-[60px] custom-scrollbar"
            disabled={isLoading}
          />
          <button
            onClick={() => handleInterpret()} disabled={isLoading || (!inputText.trim() && attachedFiles.length === 0)}
            className="bg-emerald-500 hover:bg-emerald-600 dark:hover:bg-emerald-400 disabled:opacity-30 text-white text-xs font-black uppercase tracking-[0.2em] px-10 h-[60px] brutalist-button shadow-emerald-500/20"
          >
