    ...
    <div className="h-full flex flex-col p-8 bg-transparent overflow-hidden relative">
      <div className="flex items-center justify-between mb-6 z-10 shrink-0">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Saída do Canvas</h2>
        ...
        <div className="flex items-center gap-3">
          {(result || conversationHistory.length > 0) && !isLoading && (
            <>
              <button onClick={onOptimize} disabled={isOptimizing} className={`brutalist-button text-[10px] px-3 py-2 font-black uppercase tracking-widest flex items-center gap-2 ${isOptimizing ? 'bg-purple-600/50 text-white cursor-not-allowed' : 'bg-purple-600/20 text-purple-400'}`}>
              ...
              <button onClick={handleCopy} className={`brutalist-button text-[10px] px-3 py-2 font-black uppercase tracking-widest flex items-center gap-2 ${copied ? 'bg-cyan-500 text-white' : 'bg-cyan-500/20 text-cyan-400'}`}>
              ...
              <div className="relative">
                <button onClick={() => setShowExportMenu(!showExportMenu)} className="brutalist-button text-[10px] px-3 py-2 bg-indigo-600/20 text-indigo-400 font-black uppercase tracking-widest flex items-center gap-2">
                ...
                {showExportMenu && (
                  <div className="absolute top-full mt-2 right-0 glass-panel !shadow-lg py-1 z-20 min-w-[140px] !rounded-none">
                    <button onClick={() => handleExport('markdown')} className="w-full text-left px-4 py-2 text-xs text-white font-bold hover:bg-cyan-500 transition-colors uppercase">Markdown</button>
                    <button onClick={() => handleExport('json')} className="w-full text-left px-4 py-2 text-xs text-white font-bold hover:bg-cyan-500 transition-colors uppercase">JSON</button>
                    <button onClick={() => handleExport('text')} className="w-full text-left px-4 py-2 text-xs text-white font-bold hover:bg-cyan-500 transition-colors uppercase">Txt</button>
                  </div>
                )}
              </div>
              <button onClick={() => setIsModalOpen(true)} className="brutalist-button text-[10px] px-3 py-2 bg-cyan-600/20 text-cyan-400 font-black uppercase tracking-widest flex items-center gap-2">
      ...
      <div ref={scrollContainerRef} className="flex-1 w-full glass-panel !rounded-none p-8 overflow-y-auto custom-scrollbar flex flex-col space-y-8 bg-slate-900/40">
        {conversationHistory.length === 0 && !isLoading && !result && !pendingUserMessage && (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 italic text-center px-12">
            <div className="w-16 h-16 border-4 border-cyan-500/20 mb-4 flex items-center justify-center text-cyan-500/20 font-black text-4xl">?</div>
        ...
        {conversationHistory.map((turn) => (
          <div key={turn.id} className="flex flex-col space-y-4">
            <div className="self-end max-w-[90%] bg-indigo-500/20 border-2 border-indigo-500/40 p-4 rounded-none text-sm text-white shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
              ...
              {turn.attachedFiles && turn.attachedFiles.length > 0 && (
                 <div className="mt-3 flex gap-2 flex-wrap">
                    {turn.attachedFiles.map(f => (
                       <div key={f.id} className="text-[9px] bg-indigo-500/30 px-2 py-1 border border-indigo-500/50 font-black uppercase">📎 {f.name}</div>
                    ))}
                 </div>
              )}
            </div>
            <div className="self-start max-w-[90%] bg-slate-800/80 backdrop-blur-md border-2 border-white/10 p-5 rounded-none text-sm text-slate-100 leading-relaxed shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
        ...
        {pendingUserMessage && (
          <div className="flex flex-col space-y-4 animate-pulse">
            <div className="self-end max-w-[90%] bg-indigo-500/10 border-2 border-indigo-500/30 p-4 text-sm text-slate-400">
              <p className="whitespace-pre-wrap">{pendingUserMessage}</p>
            </div>
            <div className="self-start max-w-[90%] bg-slate-800/50 border-2 border-white/5 p-5 text-sm text-slate-500 uppercase font-black tracking-widest flex items-center gap-4">
        ...
        {result && !isLoading && (
           <div className="self-start max-w-[90%] bg-red-600/20 border-2 border-red-600/40 p-5 text-sm text-red-300 font-bold shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
             <div className="flex items-center gap-2 mb-2 font-black uppercase text-xs">RELATÓRIO DE ERRO 404_GEN</div>
             {result}
           </div>
        )}
      </div>
      ...
      <div className="mt-8 flex flex-col space-y-4 shrink-0 z-10">
        ...
        <div className="flex items-center gap-4 bg-slate-900/60 backdrop-blur-xl p-4 border-2 border-white/15 shadow-[8px_8px_0_rgba(0,0,0,0.5)] focus-within:border-cyan-500/50 transition-all">
          <button onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-400 hover:text-cyan-400 transition-all border-2 border-transparent hover:border-cyan-500/30" title="Anexar">
          ...
          <textarea
            ...
            className="flex-1 bg-transparent border-none py-2 px-2 focus:outline-none text-base text-white placeholder-slate-600 font-bold resize-none h-[60px] custom-scrollbar"
            disabled={isLoading}
          />
          <button
            onClick={() => handleInterpret()} disabled={isLoading || (!inputText.trim() && attachedFiles.length === 0)}
            className="bg-cyan-500 hover:bg-cyan-400 disabled:opacity-30 text-white text-xs font-black uppercase tracking-[0.2em] px-10 h-[60px] brutalist-button"
          >
      ...
      <div className="mt-6 flex justify-between items-center text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] shrink-0 px-2">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-2"><span className={`w-2 h-2 ${isLoading ? 'bg-amber-500 animate-pulse' : 'bg-cyan-500'}`}></span>ESTADO: {isLoading ? 'OCUPADO' : 'OCIOSO'}</span>
          <span>PROTOCOLO: {conversationHistory.length} ETAPAS</span>
        </div>
        <span className="text-cyan-500/50">LINK SEGURO ESTABELECIDO</span>
      </div>
