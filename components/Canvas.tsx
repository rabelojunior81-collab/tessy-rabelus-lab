    ...
    <div className="h-full flex flex-col p-8 bg-transparent overflow-hidden relative">
      <div className="flex items-center justify-between mb-6 z-10 shrink-0">
        <div className="flex items-center gap-4">
          ...
          {hasContent && (
            <button onClick={onNewConversation} className="brutalist-button text-[10px] px-3 py-1.5 bg-red-600/15 text-red-600 dark:text-red-400 font-black uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="brutalist-button text-[10px] px-3 py-2 bg-emerald-600/15 text-emerald-600 dark:text-emerald-300 font-black uppercase tracking-widest flex items-center gap-2 border-emerald-600/25"
          >
          ...
          {(result || hasContent) && !isLoading && (
            <>
              <button onClick={onOptimize} disabled={isOptimizing} className={`brutalist-button text-[10px] px-3 py-2 font-black uppercase tracking-widest flex items-center gap-2 ${isOptimizing ? 'bg-lime-600/50 text-white cursor-not-allowed' : 'bg-lime-600/15 text-lime-600 dark:text-lime-400'}`}>
              ...
              <button onClick={handleCopy} className={`brutalist-button text-[10px] px-3 py-2 font-black uppercase tracking-widest flex items-center gap-2 ${copied ? 'bg-emerald-500 text-white' : 'bg-emerald-600/15 text-emerald-600 dark:text-emerald-400'}`}>
              ...
              <button onClick={() => setIsModalOpen(true)} className="brutalist-button text-[10px] px-3 py-2 bg-emerald-600/15 text-emerald-600 dark:text-emerald-400 font-black uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="relative" ref={exportDropdownRef}>
                <button 
                  ...
                  disabled={!hasContent}
                  className="brutalist-button text-[10px] px-3 py-2 bg-teal-600/15 text-teal-600 dark:text-teal-400 font-black uppercase tracking-widest flex items-center gap-2 disabled:opacity-50"
                >
                ...
                {showExportMenu && (
                  <div className="absolute top-full mt-2 right-0 glass-panel !shadow-[4px_4px_0_rgba(16,185,129,0.5)] py-1 z-50 min-w-[200px] !rounded-none !bg-slate-900/95 !backdrop-blur-xl !border-emerald-600 animate-in slide-in-from-top-2">
              ...
              <button 
                ...
                disabled={!hasContent}
                className="brutalist-button text-[10px] px-3 py-2 bg-emerald-600 text-white font-black uppercase tracking-widest flex items-center gap-2 disabled:opacity-50"
              >
      ...
      <div ref={scrollContainerRef} className="flex-1 w-full glass-panel !rounded-none p-8 overflow-y-auto custom-scrollbar flex flex-col space-y-8 bg-white/85 dark:bg-slate-900/30">
        {conversationHistory.length === 0 && !isLoading && !result && !pendingUserMessage && (
          <div className="h-full flex flex-col items-center justify-center text-slate-600 dark:text-slate-500 italic text-center px-12">
        ...
        {result && !isLoading && (
           <div className="self-start max-w-[90%] bg-red-600/15 dark:bg-red-600/20 border-2 border-red-600/30 dark:border-red-600/40 p-5 text-sm text-red-700 dark:text-red-300 font-bold shadow-[6px_6px_0_rgba(0,0,0,0.05)] dark:shadow-[6px_6px_0_rgba(0,0,0,0.3)]">
      ...
      <div className="mt-8 flex flex-col space-y-4 shrink-0 z-10">
        ...
        <div className="flex items-center gap-4 bg-white/85 dark:bg-slate-900/40 backdrop-blur-2xl p-4 border-2 border-emerald-600/25 shadow-[8px_8px_0_rgba(16,185,129,0.1)] dark:shadow-[8px_8px_0_rgba(0,0,0,0.5)] focus-within:border-emerald-600 transition-all">
          <button onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-600 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all border-2 border-transparent hover:border-emerald-600/30" title="Anexar">
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 dark:hover:bg-emerald-400 disabled:opacity-30 text-white text-xs font-black uppercase tracking-[0.2em] px-10 h-[60px] brutalist-button shadow-emerald-600/20"
          >
