...
const SavePromptModal: React.FC<SavePromptModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-slate-950/80 backdrop-blur-sm">
      <div className="glass-panel !rounded-none w-full max-w-md animate-in zoom-in duration-200 !bg-slate-900/60 !backdrop-blur-2xl !border-emerald-500/30">
        <div className="px-8 py-6 border-b-2 border-emerald-500/10 flex justify-between items-center bg-slate-900/40">
          <h3 className="text-xl font-black text-white uppercase tracking-tighter glow-text-green">Arquivamento</h3>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 bg-slate-900/20">
          <div>
            ...
            <input autoFocus type="text" required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Contrato V1.0..." className="w-full bg-slate-900/60 border-2 border-emerald-500/10 p-4 text-white font-bold placeholder-emerald-900/30 focus:outline-none focus:border-emerald-500 transition-all !rounded-none" />
          </div>
          <div>
